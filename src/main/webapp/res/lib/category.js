...
//        fetch('https://apps.ingoschindler.de:8443/wild/resources/users/' + encodeURIComponent(ref) + "/categories")
        fetch('/wild/resources/users/' + encodeURIComponent(ref) + "/categories")
            .then((r) => r.json())
            .then((j) => {
                vm.categories = j;
...
//                    fetch('https://apps.ingoschindler.de:8443/wild/resources/users/' + encodeURIComponent(ref) + "/categories/" + encodeURIComponent(c.id) + "/parts/")
                    fetch('/wild/resources/users/' + encodeURIComponent(ref) + "/categories/" + encodeURIComponent(c.id) + "/parts/")
                        .then((r) => r.json())
                        .then((j) => {
                            Vue.set(c, 'parts', j)
                            c.count = j.length;
